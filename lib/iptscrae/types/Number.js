...
IptNumber.prototype.not = function () {
	return new IptNumber(this._value === 0 ? 1 : 0);
};
...
IptNumber.prototype.equal = function (right) {
	if (!right.is(Types.Number)) {
		return false;
	}

	return new IptNumber(this._value === right._value ? 1 : 0);
};

IptNumber.prototype.less = function (right) {
	return new IptNumber(this._value < right.must(Types.Number)._value ? 1 : 0);
};

IptNumber.prototype.greater = function (right) {
	return new IptNumber(this._value > right.must(Types.Number)._value ? 1 : 0);
};
